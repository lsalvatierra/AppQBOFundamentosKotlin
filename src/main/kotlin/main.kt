fun main(args: Array<String>) {
    ...
    //Clases Anidadas e InnerClass
    val objFlorAnidada = Flores.ClaseAnidada()
    val saludoAnidado = objFlorAnidada.saludoClaseAnidada("Luis")
    val objFlorInterna = Flores().ClaseInterna()
    val saludoInterno = objFlorInterna.saludoClaseInterna("Luis")
    objetosDataClass()
}
...
fun herenciaDeClases(){
    ...
    val objProfesor = Profesor("Luis")
    objProfesor.dictaClase()
}

fun objetosDataClass(){
    val objAuto1 = Auto("Toyota", "Sedan", 2019, "A001")
    val objAuto2 = objAuto1.copy(marca = "Kia")
    val (marca, tipo, anio, codigo) = objAuto2
    if(objAuto1.equals(objAuto2)){
        println("son iguales")
    }
    //println("${objAuto1.toString()}")
}